...
class ImageCompressor:
    ...
    def compress_image(self, img):
        ...
        """
        oneD_image = self.convert_to_1D(img)
        km = KMeans(n_clusters=10).fit(oneD_image)
        predictions = km.predict(oneD_image)
        twoD_image = self.convert_to_2D(km.cluster_centers_[predictions])
        return twoD_image

if __name__ == '__main__':
    asdf = ImageCompressor()
    image = asdf.load_image("/Users/faizhilaly/Documents/ATCS-2021/MachineLearning/basils.jpg")
    comp_image = asdf.compress_image(image)
    asdf.plot_image_comparisons(image, comp_image)
